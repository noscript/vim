...
adjust_plines_for_skipcol(win_T *wp)
{
    ...
    int width = win_content_width(wp) - win_col_off(wp);
...
validate_cursor_col(void)
{
    ...
    off = curwin_col_off();
    col += off;
    width = win_content_width(curwin) - off + curwin_col_off2();

    // long line wrapping, adjust curwin->w_wrow
    if (curwin->w_p_wrap
	    && col >= (colnr_T)win_content_width(curwin)
	    && width > 0)
	// use same formula as what is used in curs_columns()
	col -= ((col - win_content_width(curwin)) / width + 1) * width;
    ...
    int		may_scroll)	// when TRUE, may scroll horizontally
{
    ...
    width1 = win_content_width(curwin) - extra;
    if (width1 <= 0)
    {
	// No room for text, put cursor in last char of window.
	// If not wrapping, the last non-empty line.
	curwin->w_wcol = win_content_width(curwin) - 1;
	if (curwin->w_p_wrap)
	    curwin->w_wrow = curwin->w_height - 1;
	else
	    curwin->w_wrow = curwin->w_height - 1 - curwin->w_empty_rows;
    }
    else if (curwin->w_p_wrap && win_content_width(curwin) != 0)
    {
	...
	// long line wrapping, adjust curwin->w_wrow
	if (curwin->w_wcol >= win_content_width(curwin))
	{
	    // this same formula is used in validate_cursor_col()
	    n = (curwin->w_wcol - win_content_width(curwin)) / width2 + 1;
	...
	{
	    int cols = (win_content_width(curwin) - extra);
	...
	off_right = endcol - curwin->w_leftcol - (win_content_width(curwin) - siso) + 1;
...
# endif
	{
	    ...
	    col = scol;
	    col += off;
	    width = win_content_width(wp) - off + win_col_off2(wp);

	    // long line wrapping, adjust row
	    if (wp->w_p_wrap
		    && col >= (colnr_T)win_content_width(wp)
		    && width > 0)
	    {
		// use same formula as what is used in curs_columns()
		int rowoff = ((col - win_content_width(wp)) / width + 1);
		col -= rowoff * width;
		row += rowoff;
	    }
	    col -= wp->w_leftcol;
...
# ifdef FEAT_RIGHTLEFT
	...
	*ccolp = wp->w_wincol + win_content_width(wp) - endoff;
    }
# endif
}
#endif
...
static void cursor_correct_sms(void)
{
    ...
    long    so = get_scrolloff_value();
    int	    width1 = win_content_width(curwin) - curwin_col_off();
    ...
    overlap = curwin->w_skipcol == 0 ? 0
			: sms_marker_overlap(curwin, win_content_width(curwin) - width2);
    ...
    int		byfold UNUSED)	// TRUE: count a closed fold as one line
{
    ...
    if (do_sms)
    {
	width1 = win_content_width(curwin) - curwin_col_off();
	width2 = width1 + curwin_col_off2();
    }
     ...
     */
    wrow = curwin->w_wrow;
    if (curwin->w_p_wrap && win_content_width(curwin) != 0)
    {
	validate_virtcol();
	validate_cheight();
	wrow += curwin->w_cline_height - 1 -
	    curwin->w_virtcol / win_content_width(curwin);
    }
...
adjust_skipcol(void)
{
    ...
    validate_virtcol();
    overlap = sms_marker_overlap(curwin, win_content_width(curwin) - width2);
...
scrollup_clamp(void)
{
    ...
    {
	validate_virtcol();
	start_row -= curwin->w_virtcol / win_content_width(curwin);
    }
