...
validate_cursor_col(void)
{
    ...
    off = curwin_col_off();
    col += off;
    width = W_WIDTH_INNER(curwin) - off + curwin_col_off2();

    // long line wrapping, adjust curwin->w_wrow
    if (curwin->w_p_wrap
	    && col >= (colnr_T)(W_WIDTH_INNER(curwin))
	    && width > 0)
	// use same formula as what is used in curs_columns()
	col -= ((col - (W_WIDTH_INNER(curwin))) / width + 1) * width;
    ...
    int		may_scroll)	// when TRUE, may scroll horizontally
{
	...
	curwin->w_wcol = W_WIDTH_INNER(curwin) - 1;
	if (curwin->w_p_wrap)
	    curwin->w_wrow = curwin->w_height - 1;
	else
	    curwin->w_wrow = curwin->w_height - 1 - curwin->w_empty_rows;
    }
    else if (curwin->w_p_wrap && W_WIDTH_INNER(curwin) > 0)
    {
	...
	off_right = (int)endcol - (int)(curwin->w_leftcol + W_WIDTH_INNER(curwin)
								- siso) + 1;
...
scrollup_clamp(void)
{
    ...
    {
	validate_virtcol();
	start_row -= curwin->w_virtcol / (W_WIDTH_INNER(curwin));
    }
