...
jump_to_help_window(qf_info_T *qi, int newwin, int *opened_window)
{
	...
	flags = WSP_HELP;
	if (cmdmod.cmod_split == 0 && win_split_width(curwin) != Columns
		&& win_split_width(curwin) < 80)
	    flags |= WSP_TOP;
...
qf_open_new_cwindow(qf_info_T *qi, int height)
{
    ...
    // window to the side.
    if (curtab == prevtab && win_split_width(curwin) == Columns)
