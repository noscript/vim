...
log_frame_layout(frame_T *frame)
{
    ch_log(NULL, "layout %s, wi: %d, he: %d, wwi: %d, whe: %d, id: %d",
	    ...
	    frame->fr_width,
	    frame->fr_height,
	    frame->fr_win == NULL ? -1 : win_split_width(frame->fr_win),
...
// increase current window width
    case '>':
		...
		need_mouse_correct = TRUE;
#endif
		win_setwidth(win_split_width(curwin) + (int)Prenum1);
		break;
    ...
    frame_T	*to_flatten)
{
	...
	if (new_size == 0)
	    new_size = win_split_width(oldwin) / 2;
	...
	if (win_split_width(oldwin) - new_size - 1 < p_wmw)
	    do_equal = TRUE;
	...
	if (oldwin->w_p_wfw)
	    win_setwidth_win(win_split_width(oldwin) + new_size + 1, oldwin);
					 ...
					 && oldwin->w_frame->fr_parent != NULL)
	{
		...
		if (frp->fr_win != oldwin && frp->fr_win != NULL
			&& (win_split_width(frp->fr_win) > new_size
			    || win_split_width(frp->fr_win) > win_split_width(oldwin)
							      - new_size - 1))
		{
		    do_equal = TRUE;
		    break;
	...
	*frp = *curfrp;
	...
	if (flags & (WSP_TOP | WSP_BOT))
	{
	    ...
	    win_new_width(oldwin, win_split_width(oldwin) - (new_size + 1));
	if (before)	// new window left of current one
	{
	    ...
	    wp->w_wincol = oldwin->w_wincol + win_split_width(oldwin) + 1;
	...
	{
	    wp->w_wincol = oldwin->w_wincol;
	    win_new_width(wp, win_split_width(oldwin));
	    wp->w_vsep_width = oldwin->w_vsep_width;
	}
    ...
    int		vertical)  // split windows vertically if TRUE
{
	...
	// column.
	maxcount = (win_split_width(curwin) + curwin->w_vsep_width
					     - (p_wiw - p_wmw)) / (p_wmw + 1);
    }
    else
    {
	...
	if (vertical)
	{
	    if (win_split(win_split_width(curwin) - (win_split_width(curwin) - todo)
...
snapshot_windows_scroll_size(void)
{
	...
	wp->w_last_width = win_split_width(wp);
...
# endif
	...
	if ((size_changed || scroll_changed) && v_event != NULL)
	{
	    ...
	    int width = win_split_width(wp) - wp->w_last_width;
...
frame_set_vsep(frame_T *frp, int add)
{
	...
	{
	    if (win_split_width(wp) > 0)	// don't make it negative
		win_new_width(wp, win_split_width(wp) - 1);
	    wp->w_vsep_width = 1;
	}
...
{
    wp->w_frame->fr_width = win_split_width(wp) + wp->w_vsep_width;
}
...
win_enter_ext(win_T *wp, int flags)
{
    ...
    if (win_split_width(curwin) < p_wiw && !curwin->w_p_wfw)
...
win_size_save(garray_T *gap)
{
	...
	((int *)gap->ga_data)[gap->ga_len++] =
	    win_split_width(wp) + wp->w_vsep_width;
...
frame_comp_pos(frame_T *topfrp, int *row, int *col)
{
	...
	*col += win_split_width(wp) + wp->w_vsep_width;
    }
    else
    {
	startrow = *row;
...
scroll_to_fraction(win_T *wp, int prev_height)
{
	...
	if (sline < 0)
	{
	    ...
	    if (wp->w_wrow >= wp->w_height
				       && (win_text_width(wp)) > 0)
	    {
		wp->w_skipcol += win_text_width(wp);
		--wp->w_wrow;
