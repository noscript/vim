...
set_term_and_win_size(term_T *term, jobopt_T *opt)
{
    ...
    term->tl_cols = win_content_width(curwin);
	...
	if (term->tl_cols != win_content_width(curwin))
...
position_cursor(win_T *wp, VTermPos *pos)
{
    ...
    wp->w_wcol = MIN(pos->col, MAX(0, win_content_width(wp) - 1));
...
term_update_window(win_T *wp)
{
	...
	if (wwp->w_buffer == term->tl_buffer)
	{
	    ...
	    newcols = MIN(newcols, win_content_width(wwp));
	}
	if (twp == NULL)
	    break;
    }
	...
	if (pos.row < term->tl_rows)
	{
	    int max_col = MIN(win_content_width(wp), term->tl_cols);
	...
	screen_line(wp, wp->w_winrow + pos.row
				...
				+ winbar_height(wp)
#endif
				, wp->w_wincol, pos.col, win_content_width(wp), -1,
#ifdef FEAT_PROP_POPUP
...
{
    int	    width = MAX(text_width, win_content_width(curwin));
    char_u  *textline;
    int	    fname_size;
    char_u  *p = fname;
    int	    i;
