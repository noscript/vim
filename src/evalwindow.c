...
# endif
    dict_add_number(dict, "width", win_split_width(wp));
...
# ifdef FEAT_RIGHTLEFT
    if (curwin->w_p_rl)
	col = win_split_width(curwin) - curwin->w_wcol - cursor_screen_cells() + 1;
# endif
...
f_winrestcmd(typval_T *argvars UNUSED, typval_T *rettv)
{
	...
	FOR_ALL_WINDOWS(wp)
	{
	    ...
	    buflen = vim_snprintf_safelen((char *)buf, sizeof(buf),
		"vert :%dresize %d|", winnr, win_split_width(wp));
	    ga_concat_len(&ga, buf, buflen);
	    ++winnr;
	}
    }
...
# endif
    ...
    win_new_width(curwin, win_split_width(curwin));
...
f_winwidth(typval_T *argvars, typval_T *rettv)
{
	...
	rettv->vval.v_number = win_split_width(wp);
}
#endif // FEAT_EVAL
