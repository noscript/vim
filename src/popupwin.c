...
popup_drag(win_T *wp)
{
	...
	if (width_inc != 0)
	{
	    int width = win_content_width(wp) + width_inc;
...
popup_width(win_T *wp)
{
    ...
    return win_content_width(wp) + wp->w_leftcol
...
popup_compute_clip(win_T *wp, popup_clip_T *cl)
{
    ...
    w = win_content_width(wp) - cl->clip_left_content - cl->clip_right_content;
    if (w < 0)
	w = 0;
...
popup_compute_clipwindow_offsets(win_T *wp)
{
    ...
    popup_right = wp->w_wincol + win_content_width(wp) + extra_w;
    total_h = wp->w_height + extra_h;
    total_w = win_content_width(wp) + extra_w;
    ...
    if (popup_right > cw->w_wincol + win_content_width(cw))
	wp->w_popup_rightclip = popup_right - (cw->w_wincol + win_content_width(cw));
...
popup_adjust_position(win_T *wp)
{
    ...
    {
	linenr_T ln;
	int saved_w_width = win_content_width(wp);

	if (win_content_width(wp) < maxwidth)
	    wp->w_width = maxwidth;
    ...
    {
	int len;
	int w_width = win_content_width(wp);
	...
	if (win_content_width(wp) < len + margin_width)
	{
	    wp->w_width = len + margin_width;
	    if (wp->w_maxwidth > 0 && win_content_width(wp) > wp->w_maxwidth)
	...
	if (maxwidth + right_extra >= maxspace
		&& (used_maxwidth || (minwidth > 0 && win_content_width(wp) < minwidth)))
    ...
    if (minwidth > 0 && win_content_width(wp) < minwidth)
	wp->w_width = minwidth;
    if (win_content_width(wp) > maxwidth)
    {
	if (win_content_width(wp) > maxspace && !wp->w_p_wrap)
	    // some columns cut off on the right
	    wp->w_popup_rightoff = win_content_width(wp) - maxspace;
	...
	{
	    int off = win_content_width(wp) - maxwidth;
    ...
    {
	wp->w_wincol = firstwin->w_wincol
		    + (topframe->fr_width - win_content_width(wp) - extra_width) / 2;
	if (wp->w_wincol < firstwin->w_wincol)
	    wp->w_wincol = firstwin->w_wincol;
    }
	    ...
	    || wp->w_popup_pos == POPPOS_TOPRIGHT)
    {
	int leftoff = wantcol - (win_content_width(wp) + extra_width);
	...
	want_col = left_extra + win_content_width(wp) + right_extra;
    ...
    if (wp->w_wincol + win_content_width(wp) + extra_width
				    > firstwin->w_wincol + topframe->fr_width)
	wp->w_wincol = firstwin->w_wincol + topframe->fr_width
						- win_content_width(wp) - extra_width;
    if (wp->w_wincol < firstwin->w_wincol)
	wp->w_wincol = firstwin->w_wincol;
    if (wp->w_wincol < 0)
	wp->w_wincol = 0;
	...
	wp->w_popup_prop_width = win_content_width(wp->w_popup_prop_win);
...
popup_save_layout(win_T *wp, popup_layout_T *layout)
{
    ...
    layout->width = win_content_width(wp);
...
popup_save_padding_screen(win_T *wp, popup_saved_screen_T *saved_screen)
{
    ...
    saved_screen->cols = wp->w_popup_padding[3] + win_content_width(wp)
...
f_popup_getpos(typval_T *argvars, typval_T *rettv)
{
    ...
    dict_add_number(dict, "width", win_content_width(wp) + left_extra
    ...
    dict_add_number(dict, "core_width", win_content_width(wp));
	...
	for (line = 0; line < screen_Rows; ++line)
	{
		...
		if (popup_mask[off] != popup_mask_next[off])
		{
			...
			if (wp != NULL)
			{
			    ...
			    col_done = wp->w_wincol + win_content_width(wp) - 1;
			}
		    }
		}
	    }
...
update_popups(void (*win_update)(win_T *wp))
{
	...
	for (i = cl.eff_border[0]; i < total_height - cl.eff_border[2]; ++i)
	{
		...
		int pad_col_start = wincol + wp->w_popup_border[3]
			+ wp->w_popup_padding[3] + win_content_width(wp) + wp->w_leftcol;
...
popup_create_preview_window(int info)
{
	...
	wp->w_width = win_content_width(curwin);
