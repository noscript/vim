    ...
    win_T	*tab_firstwin)
{
	...
	for (wp = tab_firstwin; wp != NULL; wp = wp->w_next)
	{
	    ...
	    if (win_content_width(wp) < Columns && (fprintf(fd,
		   "exe 'vert :%dresize ' .. ((&columns * %ld + %ld) / %ld)",
			    n, (long)win_content_width(wp), Columns / 2, Columns) < 0
						  || put_eol(fd) == FAIL))
		return FAIL;
	}
    }
...
# endif
	...
	{
	    if (!wp->w_p_wrap && wp->w_leftcol > 0 && win_content_width(wp) > 0)
	    {
		if (fprintf(fd,
			  "  var c: number = %ld - ((%ld * winwidth(0) + %ld) / %ld)",
			    (long)wp->w_virtcol + 1,
			    (long)(wp->w_virtcol - wp->w_leftcol),
			    (long)win_content_width(wp) / 2, (long)win_content_width(wp)) < 0
			|| put_eol(fd) == FAIL
