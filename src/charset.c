	...
	for (i = 0; i < cts->cts_text_prop_count; ++i)
	{
		...
		if (p != NULL)
		{
			...
			cells = text_prop_position(wp, tp, vcol,
			     (vcol + size) % (win_display_width(wp) - col_off) + col_off,
...
# ifdef FEAT_LINEBREAK
	...
	int	width2 = win_display_width(wp) - col_off_prev + win_col_off2(wp);
...
#  endif
	...
	int	head_prev = 0;
	if (wcol >= win_display_width(wp))
	{
	    wcol -= win_display_width(wp);
	    col_off_prev = win_display_width(wp) - width2;
	...
	if (wcol + size > win_display_width(wp))
	{
		...
		int prev_rem = win_display_width(wp) - wcol;
...
#  endif
	...
	*headp = head;
     ...
     * needs a break here.
     */
    if (wp->w_p_lbr && wp->w_p_wrap && win_display_width(wp) != 0
	...
	colnr_T colmax = (colnr_T)(win_display_width(wp) - numberextra - col_adj);
	if (vcol >= colmax)
	{
	    colmax += col_adj;
...
in_win_border(win_T *wp, colnr_T vcol)
{
    ...
    if (win_split_width(wp) == 0)	// there is no border
	return FALSE;
    width1 = win_text_width(wp);
