	...
	for (i = 0; i < cts->cts_text_prop_count; ++i)
	{
		...
		if (p != NULL)
		{
			...
			cells = text_prop_position(wp, tp, vcol,
			     (vcol + size) % (wp->w_width - wp->w_p_rmar - col_off) + col_off,
...
# ifdef FEAT_LINEBREAK
	...
	int	width2 = wp->w_width - wp->w_p_rmar - col_off_prev + win_col_off2(wp);
...
#  endif
	...
	int	head_prev = 0;
	if (wcol >= wp->w_width - wp->w_p_rmar)
	{
	    wcol -= wp->w_width - wp->w_p_rmar;
	    col_off_prev = wp->w_width - wp->w_p_rmar - width2;
	...
	if (wcol + size > wp->w_width - wp->w_p_rmar)
	{
		...
		int prev_rem = wp->w_width - wp->w_p_rmar - wcol;
...
#  endif
	...
	*headp = head;
     ...
     * needs a break here.
     */
    if (wp->w_p_lbr && wp->w_p_wrap && wp->w_width - wp->w_p_rmar != 0
	...
	colnr_T colmax = (colnr_T)(wp->w_width - wp->w_p_rmar - numberextra - col_adj);
	if (vcol >= colmax)
	{
	    colmax += col_adj;
...
in_win_border(win_T *wp, colnr_T vcol)
{
    ...
    if (wp->w_width - wp->w_p_rmar == 0)	// there is no border
	return FALSE;
    width1 = wp->w_width - wp->w_p_rmar - win_col_off(wp);
