...
win_redr_status(win_T *wp, int ignore_pum UNUSED)
{
	...
	this_ru_col = ru_col - (cmdline_width - win_split_width(wp));
	n = (win_split_width(wp) + 1) / 2;
	if (this_ru_col < n)
	    this_ru_col = n;
	if (this_ru_col <= 1)
	{
...
# endif
	...
	for (p = text; *p != NUL; )
	{
	    ...
	    c_len = (*mb_ptr2len)(p);
	    if (col + cells > win_display_width(wp))
		break;
# ifdef FEAT_RIGHTLEFT
	    if (wp->w_p_rl)
		idx = off + win_split_width(wp) - col - cells;
# endif
	    ScreenLines[idx] = *p;
	    if (enc_utf8)
	    {
...
# endif
	...
	int n = win_display_width(wp) - col;
...
redraw_win_toolbar(win_T *wp)
{
    ...
    for (menu = wp->w_winbar->children;
			  menu != NULL && col < win_display_width(wp); menu = menu->next)
    {
	space_to_screenline(off + col, fill_attr);
	if (++col >= win_display_width(wp))
	    break;
	if (col > 1)
	{
	    space_to_screenline(off + col, fill_attr);
    ...
    while (col < win_display_width(wp))
    {
	space_to_screenline(off + col, fill_attr);
	++col;
    }
    ...
    screen_line(wp, wp->w_winrow, wp->w_wincol, win_display_width(wp),
							  win_display_width(wp), -1, 0);
...
#  define RL_MEMSET(p, v, l) \
	...
	if (wp->w_p_rl) \
	    for (ri = 0; ri < (l); ++ri) \
	       ScreenAttrs[off + (win_split_width(wp) - (p) - (l)) + ri] = v; \
	 else \
	    for (ri = 0; ri < (l); ++ri) \
...
# ifdef FEAT_RIGHTLEFT
	    if (wp->w_p_rl)
		idx = off + win_split_width(wp) - i - 1 - col;
	    else
# endif
		idx = off + col + i;
	    if (enc_utf8)
    ...
    // text
    RL_MEMSET(col, HL_ATTR(HLF_FL), win_split_width(wp) - col);

# ifdef FEAT_SIGNS
    // If signs are being displayed, add two spaces.
    if (signcolumn_on(wp))
    {
	len = win_split_width(wp) - col;
	if (len > 0)
	{
	    if (len > 2)
		len = 2;
#  ifdef FEAT_RIGHTLEFT
		...
		copy_text_attr(off + win_split_width(wp) - len - col,
...
# ifdef FEAT_RIGHTLEFT
	    if (wp->w_p_rl)
		// the line number isn't reversed
		copy_text_attr(off + win_split_width(wp) - len - col, buf, len,
							     HL_ATTR(HLF_FL));
	    else
# endif
...
# ifdef FEAT_RIGHTLEFT
	if (wp->w_p_rl)
	    idx = off + win_split_width(wp) - 1 - col;
# endif
				    ...
				    >= ml_get_buf_len(wp->w_buffer, lnume))))))
	{
		...
		if (wp->w_old_cursor_fcol + txtcol < (colnr_T)win_split_width(wp))
		{
		    if (wp->w_old_cursor_lcol != MAXCOL
			     && wp->w_old_cursor_lcol + txtcol
						       < (colnr_T)win_split_width(wp))
			len = wp->w_old_cursor_lcol;
		    else
			len = win_split_width(wp) - txtcol;
		...
		RL_MEMSET(txtcol, HL_ATTR(HLF_V), win_split_width(wp) - txtcol);
	    }
	}
    }
...
# ifdef FEAT_SYN_HL
	...
	while (j > -1)
	{
		...
		txtcol -= wp->w_leftcol;
	    if (txtcol >= 0 && txtcol < win_split_width(wp))
...
# endif
    ...
    screen_line(wp, row + W_WINROW(wp), wp->w_wincol, win_split_width(wp), win_split_width(wp),
	    -1, 0);
...
# endif
    ...
    // width.
    if (wp->w_skipcol > 0 && win_display_width(wp) > win_col_off(wp))
    {
	int w = 0;
	int width1 = win_text_width(wp);
...
# endif
	...
	else if (dy_flags & DY_TRUNCATE)	// 'display' has "truncate"
	{
	    ...
	    screen_puts_len(fillbuf,
			    (win_display_width(wp) > 2 ? 2 : win_display_width(wp)) * charlen,
