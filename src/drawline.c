...
margin_columns_win(win_T *wp, int *left_col, int *right_col)
{
    ...
    width1 = win_display_width(wp) - cur_col_off;
	...
	int	    do_skip)	    // skip_cells is not zero
{
    ...
    int	    room = win_display_width(wp) - col_with_padding;
	...
	{
	    before = 0;
	    after = win_display_width(wp) - cells - win_col_off(wp) - padding;
	    if (after < 0)
	    {
	...
	else
	{
			      ...
			      || (room < PROP_TEXT_MIN_CELLS && wp->w_p_wrap)))
		{
		    ...
		    before = win_display_width(wp) - col_off - strsize + room;
		    if (before < 0)
			before = 0;
		    else
			n_used = *n_extra;
...
wlv_screen_line(win_T *wp, winlinevars_T *wlv, int clear_end)
{
	...
	if (wp->w_p_nu && wp->w_p_rnu)
	    ...
	    // "123<<<xt".
	    while (skip < win_split_width(wp) && VIM_ISDIGIT(ScreenLines[off]))
	    {
		++off;
		++skip;
	    }

	for (int i = 0; i < 3 && i + skip < win_split_width(wp); ++i)
	{
    ...
    screen_line(wp, wlv->screen_row, wp->w_wincol, wlv->col,
		    clear_end ? win_split_width(wp) : -win_split_width(wp),
...
draw_screen_line(win_T *wp, winlinevars_T *wlv)
{
    ...
    wcol =
# ifdef FEAT_RIGHTLEFT
	wp->w_p_rl ? win_split_width(wp) - wlv->col - 1 :
# endif
	wlv->col;
...
# endif
    ...
    if (((wp->w_p_cuc
		    ...
		    && (int)wp->w_virtcol <
			 (long)win_split_width(wp) * (wlv->row - wlv->startrow + 1) + v
			 && wlv->lnum != wp->w_cursor.lnum)
		|| wlv->draw_color_col
# ifdef LINE_ATTR
		|| wlv->line_attr != 0
...
# endif
	...
	if (text_props != NULL)
	{
	    ...
	    int text_width = win_text_width(wp);
...
# endif
			    if ((right || above || below || !wrap
					    || padding > 0) && win_display_width(wp) > 2)
			    {
...
# endif
				...
				if (lcs_eol_one < 0
					&& wp->w_p_wrap
					&& wlv.col
					       + wlv.n_extra - 2 > win_display_width(wp))
			...
			if (bail_out)
			   ...
			   && ((*ptr != NUL && ptr[mb_ptr2len(ptr)] == NUL)
			       || (!wp->w_p_wrap && wlv.col == win_display_width(wp) - 1)))
		{
...
# endif
		    ...
		    if (c == TAB && wlv.n_extra + wlv.col > win_display_width(wp))
# ifdef FEAT_VARTABS
...
# ifdef FEAT_RIGHTLEFT
	    if (wp->w_p_rl)
		wp->w_wcol = win_split_width(wp) - wlv.col + wlv.boguscols - 1;
	    else
# endif
...
#endif
		{
		    if (wlv.col >= win_display_width(wp))
			n = -1;
		}
		if (n != 0)
		{
...
# endif
						      ...
						      + screen_Columns) == 2)))
		{
		    ...
		    if (screen_cur_col != win_split_width(wp))
