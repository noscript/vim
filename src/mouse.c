...
get_fpos_of_mouse(pos_T *mpos)
{
    ...
    if (col >= win_content_width(wp))	// In vertical separator line
	return IN_SEP_LINE;
...
do_mousescroll(cmdarg_T *cap)
{
	...
	long step = (mouse_hor_step < 0 || shift_or_ctrl)
					    ? win_content_width(curwin) : mouse_hor_step;
...
# endif
	...
	if (row >= wp->w_height)		// In (or below) status line
	{
	    ...
	    on_status_line = 0;
	if (col >= win_content_width(wp))		// In separator line
	{
	    on_sep_line = col - win_content_width(wp) + 1;
	    dragwin = wp;
	}
	else
	    on_sep_line = 0;
...
# ifdef FEAT_RIGHTLEFT
			    wp->w_p_rl ? col < win_content_width(wp) - wp->w_p_fdc :
# endif
...
# endif
	...
	if (dragwin != NULL)
	{
	    ...
	    count = col - dragwin->w_wincol - win_content_width(dragwin) + 1
								- on_sep_line;
    ...
    if (
# ifdef FEAT_RIGHTLEFT
	    curwin->w_p_rl ? col < win_content_width(curwin) - curwin->w_p_fdc :
# endif
...
#ifdef FEAT_RIGHTLEFT
    if (win->w_p_rl)
	col = win_content_width(win) - 1 - col;
#endif
...
# endif
	...
	{
	    int width1 = win_content_width(win) - win_col_off(win);
	...
	if (col < off)
	    col = off;
	col += row * (win_content_width(win) - off);
...
# endif
	if (row < height)
	{
	    ...
	    row -= top_off;
	    col -= left_off;
	    if (row >= 0 && row < wp->w_height && col >= 0 && col < win_content_width(wp))
	    {
