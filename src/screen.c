    ...
    hlf_T	hl)
{
    ...
    if (wp->w_p_rl)
    {
	int margin_width = win_split_width(wp) - win_display_width(wp);

	screen_fill(W_WINROW(wp) + row, W_WINROW(wp) + endrow,
		wp->w_wincol, wp->w_wincol + margin_width,
		' ', ' ', draw_margin ? win_attr : attr);
	screen_fill(W_WINROW(wp) + row, W_WINROW(wp) + endrow,
		wp->w_wincol + margin_width,
    ...
    {
	screen_fill(W_WINROW(wp) + row, W_WINROW(wp) + endrow,
		wp->w_wincol + n, (int)wp->w_wincol + win_display_width(wp),
		c1, c2, attr);
	screen_fill(W_WINROW(wp) + row, W_WINROW(wp) + endrow,
		(int)wp->w_wincol + win_display_width(wp), (int)W_ENDCOL(wp),
		' ', ' ', draw_margin ? win_attr : attr);
    }
...
#endif
	if (endcol > win_display_width(wp))
	    endcol = win_display_width(wp);
    if (endcol > Columns)
	endcol = Columns;
