	...
	int	attr)
{
    ...
    if (nn > win_content_width(wp))
	nn = win_content_width(wp);
...
compute_foldcolumn(win_T *wp, int col)
{
    ...
    int n = win_content_width(wp) - (col + wmw);
    ...
    int		draw_ruler)	// TRUE or FALSE
{
	...
	maxwidth = in_status_line ? win_content_width(wp) : cmdline_width;
...
linecopy(int to, int from, win_T *wp)
{
    ...
    mch_memmove(ScreenLines + off_to, ScreenLines + off_from,
	    win_content_width(wp) * sizeof(schar_T));
    if (enc_utf8)
    {
	int	i;

	mch_memmove(ScreenLinesUC + off_to, ScreenLinesUC + off_from,
		win_content_width(wp) * sizeof(u8char_T));
	for (i = 0; i < p_mco; ++i)
    ...
    mch_memmove(ScreenAttrs + off_to, ScreenAttrs + off_from,
	    win_content_width(wp) * sizeof(sattr_T));
    mch_memmove(ScreenCols + off_to, ScreenCols + off_from,
	    win_content_width(wp) * sizeof(colnr_T));
}
...
setcursor_mayforce(int force)
{
	...
	windgoto(W_WINROW(curwin) + curwin->w_wrow,
		...
		curwin->w_p_rl ? ((int)win_content_width(curwin) - curwin->w_wcol
					    - cursor_screen_cells()) :
#endif
					    curwin->w_wcol));
    }
    ...
    int		clear_attr)
{
     ...
     * scroll-up .
     */
    if (scroll_region || win_content_width(wp) != topframe->fr_width)
    {
    ...
    win_T	*wp)	    // NULL or window to use width from
{
    ...
    if (wp != NULL && win_content_width(wp) != topframe->fr_width && *T_CSV == NUL)
    {
    ...
    if (off + row > 0 || (wp != NULL && win_content_width(wp) != topframe->fr_width))
    ...
    {
	if (wp != NULL && win_content_width(wp) != topframe->fr_width)
	{
		...
		lineclear(LineOffset[j] + wp->w_wincol,
			win_content_width(wp), clear_attr);
	    else
		lineinvalid(LineOffset[j] + wp->w_wincol, win_content_width(wp));
	    LineWraps[j] = FALSE;
	}
	else
	{
