 ...
 *	else if 'wrapmargin' option is set, use win_content_width(curwin) - 'wrapmargin'
    ...
    int		ff)	// force formatting (for "gq" command)
{
	...
	textwidth = win_content_width(curwin) - curbuf->b_p_wm;
    ...
    if (ff && textwidth == 0)
    {
	textwidth = win_content_width(curwin) - 1;
	if (textwidth > 79)
	    textwidth = 79;
    }
    return textwidth;
