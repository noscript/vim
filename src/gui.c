...
{
    int		midcol = curwin->w_wincol + win_split_width(curwin) / 2;
    int		has_midcol = (wp->w_wincol <= midcol
				     && wp->w_wincol + win_split_width(wp) >= midcol);
	...
	{
	    if (which == SBAR_RIGHT ? wp->w_wincol + win_split_width(wp) != Columns
								: !has_midcol)
		enable = FALSE;
	}
    }
...
xy2win(int x, int y, mouse_find_T popup)
{
    ...
    else if (!(State & MODE_CMDLINE) && wp->w_vsep_width > 0 && col == win_split_width(wp)
