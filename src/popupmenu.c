				...
				// to use the cursor column
{
	...
	pum_win_width = win_split_width(curwin);
...
#ifdef FEAT_RIGHTLEFT
	    if (pum_rl)
		cursor_col = curwin->w_wincol + win_split_width(curwin) - wcol - 1;
	    else
#endif
...
pum_redraw(void)
{
	...
	if (pum_rl)
	{
	    if (pum_col < curwin->w_wincol + win_split_width(curwin) - 1
							    - pum_border)
