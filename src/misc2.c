    ...
    colnr_T	wcol_arg)	// column to move to (can be negative)
{
    ...
    {
	int		width = win_split_width(curwin) - curwin_col_off();
	chartabsize_T	cts;

	if (finetune
		&& curwin->w_p_wrap
		&& win_split_width(curwin) != 0
		&& wcol >= (colnr_T)width
		&& width > 0)
	{
	...
	else
	{
	    ...
	    if (b > 0 && b < (MAXCOL - 2 * win_split_width(curwin)))
		pos->coladd = b;
...
set_leftcol(colnr_T leftcol)
{
    ...
    long lastcol = curwin->w_leftcol + win_text_width(curwin) - 1;
    validate_virtcol();
